...
On one virtual machine, install Windows Server 2012 R2, or Windows Server 2016 Technical Preview 4 or later, to create a computer called *CORPDC*.
...
### Add roles to establish a domain controller for demonstration purposes
...
> [!NOTE]  If you already have a domain to use as your CORP domain, and that domain uses a Windows Server 2012 R2 or later as its domain controller, you can skip to the section [Create additional users and groups for demonstration purposes](#create-additional-users-and-groups-for-demonstration-purposes).

Create a group needed for auditing purposes by Active Directory, if the group does not already exist.

The name of the group must be the NetBIOS domain name followed by three dollar signs: *CONTOSO$$$*. The group scope must be *Domain local*, and the group type *Security*. This is necessary so that groups can be created in the PRIV forest in a later step.
...
Note that in some cases the group may already exist - this is normal if the domain was also used in AD migration scenarios.
...
2. Type the following commands.  The password 'Pass@word1' is just an example and you should choose a different password string.
...
You need to enable auditing in existing forests in order for the PAM configuration to be established with those forests.  
...
This will restart the domain controller, CORPDC. For further information on this registry setting, see this [knowledge base article](http://support.microsoft.com/kb/322970).

## Prepare a CORP workstation and resource

If you do not already have a workstation computer joined to the domain, follow these instructions to prepare a workstation.  If you do have a workstation joined to the domain, skip to the section [Create a resource for demonstration purposes](#create-a-resource-for-demonstration-purposes) below.
...
1. Connect to the workstation CORPWKSTN. Click the **Switch user** icon, the **Other user**. Make sure that the user CONTOSO\\Jen can log into CORPWKSTN.

2. Create a new folder named *CorpFS* and share it with the *CorpAdmins* group.

3. Open PowerShell as an administrator.

4. When the window opens, type the following commands.
