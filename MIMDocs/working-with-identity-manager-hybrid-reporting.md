...
3.  Install the Microsoft Identity Manager reporting agent:
    ...
    1.  Download the [MicrosoftEntraConnectHealthAgentSetup.exe](https://download.microsoft.com/download/9/F/5/9F53F124-C990-42D2-8A32-6D352A67762B/MicrosoftEntraConnectHealthAgentSetup.exe) to Microsoft Identity Manager Service server.
    2.  Run `MicrosoftEntraConnectHealthAgentSetup.exe` 
    3.  Run the agent installer.

    4.  Make sure that the Microsoft Entra Connect Health Agent service is running
...
2.  Click the **Microsoft Entra ID** icon.
...
If you want to stop uploading reporting audit data from Microsoft Identity Manager to Microsoft Entra ID, uninstall the hybrid reporting agent. Use the Windows **Add or Remove Programs** tool to uninstall Microsoft Identity Manager Hybrid Reporting.
