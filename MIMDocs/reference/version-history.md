...
The Microsoft Identity Manager team regularly releases updates. This article is designed to help you keep track of the versions that have been released. You can then determine whether you're already on the latest service pack and hotfix update version, or if you need to upgrade.
...
This hotfix contains updates for the MIM PAM components, MIM Service and Portal, MIM Synchronization Service, and also contains cumulative updates to MIM components from the previous hotfixes for MIM 2016 SP2.
...
>[!NOTE]
> Make sure to read [MIM deployment guide](../microsoft-identity-manager-deploy.md) for updated list of prerequisites and limitations related to TLS 1.2 only environments, Group Managed Service Accounts support, and the [upgrade path from previous MIM and FIM versions](../microsoft-identity-manager-2016-service-pack-2-upgrade-path.md).

A Service Pack 2 (SP2) rollup package (build 4.6.34.0) is available for Microsoft Identity Manager (MIM) 2016. It's a cumulative update that replaces earlier MIM 2016 SP1 updates 4.4.1302.0 through build 4.5.412.0.
...
> [!IMPORTANT]
>- RCDC form for group objects fails to render when 'displayedOwner' attribute value isn't populated. You won't be able to edit/view groups when such an error occurs.<br>
...
This hotfix contains updates for MIM Sync, MIM Service, MIM Portal, MIM CM, and PAM components. It's a cumulative update that replaces earlier MIM 2016 SP1 updates 4.4.1302.0 through build 4.5.286.0.
...
#### Privilege Access Management 
- PAM REST API couldn't be started because it couldn't load file or assembly

#### Microsoft Identity Portal
- Portals are displayed with an incorrect table length
...
- MIM Service installation failed with Database Upgrade error: Can't insert the value NULL into column 'Name' if not default database name is used
- In hotfix cases the Microsoft 365 setting would be cleared, The encrypted password for the MIM Service’s Exchange Online mailbox isn't changed
...
- Change Mode PAM accounts aren't exposed for configuration
...
- Set calculation fails to reflect correct membership. You can't delete a binding for an attribute if it's referenced in a dynamic set or group filter.
- The MIM Service didn't work for the Request Approval scenario with Exchange Online where approvals are responded to through the MIM Add-in for Outlook.
- The msidmPhoneGatePhoneNumber attribute without a Country Code doesn't use the DefaultCountryCode value in MFASettings.xml.
...
- New-PAMDomainConfiguration PowerShell cmdlet sets an incorrect value for domain trust configuration resulting in error (This unknown request parameter can't be processed).
...
- Word function doesn't return empty string if string contains less than number of words.
...
- The workflow runs an enumerate resource activity to query MIM and it's failing intermittently.

#### Privilege Access Management

- Get-PAMRequestToApprove commandlet doesn't return candidates if the approver not in the role to approve.
- Related MPRs and PAM Navigation Bar node are enabled even when PAM isn't installed.
- MIM Service throws exception and logs warning from Domain Configuration synchronizer for PAM.

#### Microsoft Identity Portal

- When using Firefox to access the filter builder on the MIM portal you're unable to use the inner controls (drop-down boxes, text boxes, and so on). They can't be selected until you first right click (that is,  clicking to display the context menu).
...
- Popup had fixed size and edit controls wasn't proper sized.
...
- UI Language doesn't check country code equality.
...
- **Image file format validation on upload:** MIM is now able to validate the file format of images when they're uploaded to the portal.
...
1. Uninstall the Microsoft Identity Manager component you're upgrading
